...
interface ProductCardProps {
  ...
  // اختیاری است؛ اگر ارسال نشود، از روی price و discountPercent محاسبه می‌شود.
  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      {Array.from({ length: emptyStars }).map((_, i) => (
        <svg
          key={`empty-${i}`}
          className="w-3 h-3 sm:w-4 sm:h-4 text-zinc-600 fill-zinc-600"
          viewBox="0 0 20 20"
          aria-hidden="true"
        >
          <path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z" />
...
export default function ProductCard({
  ...
  title,
  price,
  discountedPrice: providedDiscountedPrice,
  discountPercent = 0,
  seller,
  stock = 0,
  isFeatured = false,
  ...
  /*
   * اگر discountedPrice از ShopPage ارسال شده باشد،
   * همان را استفاده می‌کنیم.
   *
   * اگر ارسال نشده باشد، قیمت را همینجا محاسبه می‌کنیم.
   */
  const finalDiscountedPrice =
    hasDiscount
      ? providedDiscountedPrice !== undefined
        ? Math.round(providedDiscountedPrice)
        : Math.round(
            price -
              (price * safeDiscountPercent) / 100
          )
      : price;
